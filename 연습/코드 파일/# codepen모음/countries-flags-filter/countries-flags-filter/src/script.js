...
countries.forEach(function (country) {
	let tmp = document.createElement("figure");
	tmp.classList.add("grid-item");
	let tmpImg = document.createElement("img");
	tmpImg.src = "https://flagsapi.com/" + country["alpha-2"] + "/flat/64.png";
	console.log(tmpImg);
	tmpImg.title = "Flag of " + country.name;
	let figCap = document.createElement("figcaption");
	figCap.textContent = country.name;
	tmp.appendChild(tmpImg);
	tmp.appendChild(figCap);
	countriesGrid.appendChild(tmp);
	country.gridElement = tmp;
});

search.addEventListener("input", function (e) {
	let inp = search.value;
	console.log(inp);
	let fittingCountries = countries.filter(
		(item) => item.name.toUpperCase().indexOf(inp.toUpperCase()) !== -1
	);
	countries.forEach((item) => (item.gridElement.style.display = "none"));
	fittingCountries.forEach((item) => (item.gridElement.style.display = "block"));
});
